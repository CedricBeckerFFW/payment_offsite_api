...
class RedirectAccessCheck implements AccessInterface{
   ...
   *   The access result.
   */
  public function access(Route $route, AccountInterface $account, PaymentInterface $payment) {
    return AccessResult::allowedIf($account->id() == $payment->getOwnerId()
      && get_parent_class($payment->getPaymentMethod()) == 'Drupal\payment_offsite_api\Plugin\Payment\Method\PaymentMethodBaseOffsite'

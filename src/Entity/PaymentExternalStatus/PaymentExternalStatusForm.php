...
namespace Drupal\payment_offsite_api\Entity\PaymentExternalStatus;
 ...
 */
class PaymentExternalStatusForm extends EntityForm {
   ...
   */
  protected $paymentExternalStatusStorage;
  ...
  public function __construct(TranslationInterface $string_translation, EntityStorageInterface $payment_status_storage, PluginSelectorManagerInterface $plugin_selector_manager, PluginTypeManagerInterface $plugin_type_manager) {
    $this->paymentExternalStatusStorage = $payment_status_storage;
  ...
  public static function create(ContainerInterface $container) {
    ...
    return new static(
      ...
      $entity_manager->getStorage('payment_status'),
  ...
  public function save(array $form, FormStateInterface $form_state) {
    $payment_external_status = $this->getEntity();
    $payment_external_status->save();
    drupal_set_message($this->t('@label has been saved.', array(
      '@label' => $payment_external_status->label()
    )));
  ...
  public function paymentStatusIdExists($id) {
    return (bool) $this->paymentExternalStatusStorage->load($id);
  }
  ...
  protected function getParentPaymentStatusSelector(FormStateInterface $form_state) {
    ...
    if ($form_state->has($key)) {
      ...
      $plugin_selector->setSelectablePluginType($this->pluginTypeManager->getPluginType('payment_external_status'));

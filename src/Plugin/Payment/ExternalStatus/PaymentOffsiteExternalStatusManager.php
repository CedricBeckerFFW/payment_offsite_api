 ...
 *
 * @see \Drupal\payment_offsite_api\Plugin\Payment\ExternalStatus\PaymentStatusInterface
 */
class PaymentOffsiteExternalStatusManager extends DefaultPluginManager implements PaymentOffsiteExternalStatusManagerInterface, FallbackPluginManagerInterface {
  ...
  protected $defaults = array(
    ...
    'class' => DefaultPaymentOffsiteExternalStatus::class,
  );
  ...
  public function __construct(CacheBackendInterface $cache_backend, ModuleHandlerInterface $module_handler, ClassResolverInterface $class_resolver, TranslationInterface $string_translation) {
    $this->alterInfo('payment_offsite_external_status');
    $this->setCacheBackend($cache_backend, 'payment_offsite_external_status', ['payment_offsite_external_status']);
    $this->classResolver = $class_resolver;
    $this->discovery = new YamlDiscovery('payment.status', $module_handler->getModuleDirectories());
    $this->discovery = new ContainerDerivativeDiscoveryDecorator($this->discovery);
    $this->factory = new ContainerFactory($this, PaymentOffsiteExternalStatusInterface::class);
